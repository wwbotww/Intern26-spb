...
AGENT_PUBLIC_AUTHORITY="${AGENT_PUBLIC_ORIGIN#https://}"
export AGENT_PUBLIC_AUTHORITY
...
envsubst '${AGENT_PUBLIC_AUTHORITY} ${AGENT_UI_MODE}' < /opt/agent/nginx/nginx.conf.template > /tmp/agent-nginx.conf
