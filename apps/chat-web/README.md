...
| [AgentSlotForm.vue](src/components/AgentSlotForm.vue)、[agent-slot-input.ts](src/agent-slot-input.ts) | 消费动态 required_inputs，将用户选择序列化为合法消息 |
...
`session_ref` 用于检测身份漂移，不是授权凭证。刷新恢复只重建公开快照，
继续执行仍由服务器验证 owner、TTL 和幂等键；没有跨设备历史查询接口。
