...
V1 设备模式不等于 V1 数据表：catalog_v2 配置下由薄包装器借用统一查询核心。
旧协议对无金额状态/多候选的表达限制见[价格消费合同](integrations/product-price.md#35-设备报价与旧协议投影)；
不新增生鲜 V1 模式，也不向旧客户端直接返回商品 Agent DTO。
...
| 方法 | 路径 | 用途 | 鉴权 |
|---|---|---|---|
| `POST` | `/v1/chat` | 政策或设备价格查询，支持 JSON/SSE | 是 |
