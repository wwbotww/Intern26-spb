...
- [6A-3](docs/agent-kernel-phase6a3-controlled-deployment.md) 已完成独立受控入口 / 冻结镜像、
  ...
  当时价格库为空；2026-09-10 复查已存在价格数据，新价格示例与政策示例均成功。
  [首页体验修复](docs/agent-homepage-quality.md)新增悬浮自动关闭通知与 19 项 Web 回归，当前 Web `89 passed`，
  本机浏览器验证通知不占布局高度。业务覆盖 / 完整页面验收 / 真实物流接口 / holdout 保留为独立缺口；
