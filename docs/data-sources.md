...
代码默认 device_v1 的 Agent V2 通过兼容 Tool 消费价格数据 V1；已限定发布的 catalog_v2 配置
通过统一报价核心消费 V2。默认值与实际部署配置不能混用。
...
catalog_v2 已使用统一只读查询核心，设备与生鲜分别匹配、共用证据与结果边界。
设备使用可信标准商品关联，生鲜允许仅有来源归一属性；该配置的 V1 HTTP 薄适配也不读旧表。
默认 device_v1 兼容路径仍保留；完整五品牌验收与旧 SQL 退出是遗留，不宣称代码已彻底删除旧表依赖。
