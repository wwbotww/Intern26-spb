...
代码库已在 `assistant-api` 隔离路径完成阶段 1–2 LangGraph Agent 工程切片：Domain 契约
...
Tool 收据。该图尚未挂载到 FastAPI；因此当前运行拓扑、`/v1` 单轮语义和
`memory=disabled` 健康状态保持不变。LangGraph import 继续由架构测试限制在 Workflow
