...
1. 规范化品牌、型号、容量、网络制式和其他规格；
2. 通过参数化 SQL 按品牌、型号关键词和有效状态缩小候选集；
3. 优先匹配标准型号或 SKU 标识，再对名称使用词元和模糊相似度排序；
4. 使用容量等结构化字段过滤或提升排序；
5. 返回达到阈值的多个候选，不强行选择唯一 SKU；
6. 默认使用当前价格；只有用户明确询问历史时才读取历史记录；
7. 每个结果展示来源、观察时间和在售状态，并标记为“参考价格”。
