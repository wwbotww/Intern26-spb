...
首页上线反馈补充（2026-09-10）：将通知从内容流拆成有生命周期的悬浮层，区分可自动消失的
提示和必须保留的身份操作；新增 19 项 Web 回归，本机浏览器验证聊天区高度不变及通知消失。
通过 SSE 状态、RAG 拒答原因、检索候选与价格库覆盖定位两个失败示例：一个是“相关但不足以回答”，
另一个是指定型号无有效报价。价格库已非空，不能继续沿用上一发布时点的空库结论。
替代示例实际返回政策引用和价格候选，未调低防幻觉 / 型号约束、未伪造答案。可作为
“将服务健康、业务可回答性和用户体验分开验收”的面试案例，详见[首页质量复盘](agent-homepage-quality.md)。
